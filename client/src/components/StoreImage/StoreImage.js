    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          fetch('http://localhost:5000/banner', { // replace with your Flask server's endpoint
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({ gambar: name, url_gambar: downloadURL }), // use the correct keys here
          })
